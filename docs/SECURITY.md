...
| `contact_leads` | ชื่อ, บริษัท, เบอร์/อีเมล, ข้อความ, IP — จากฟอร์มบนหน้าเว็บสาธารณะ |
...
`GET /` และ `POST /contact` เป็นสองเส้นทางเดียวที่เปิดให้คนนอกองค์กร (ADR-029)
ที่เหลืออยู่หลัง `auth` ทั้งหมด และ `texson:security-check` ตรวจข้อนี้ทุกครั้ง —
allowlist ระบุสองเส้นทางนี้ไว้ชัดแจ้ง ไม่ได้ปิดการตรวจ

ฟอร์มติดต่อเป็นจุดเดียวที่คนนอกเขียนข้อมูลลงฐานได้ จึงคุมไว้สี่ชั้น:

| ความเสี่ยง | ที่ทำ |
|---|---|
| ยิงฟอร์มรัว | `throttle:5,1` ต่อ IP เท่ากับหน้าเข้าสู่ระบบ |
| บอตกรอกอัตโนมัติ | ช่องล่อ `website` ที่ซ่อนจากสายตา + กฎ `prohibited` |
| ยัดข้อมูลเข้าฐาน | จำกัดความยาวทุกช่อง · `service_interest` เป็น enum ปิด |
| XSS | Blade escape ตามปกติ มีเทสต์ยิง `<script>` เข้ามายืนยัน |

หน้า landing ไม่ดึงข้อมูลภายในองค์กรมาแสดงเลย — เบอร์และอีเมลมาจากตาราง `settings`
เท่านั้น และมีเทสต์ยืนยันว่าชื่อ/อีเมลพนักงานไม่หลุดออกไป
